...
if($idProducto==0){
  ...
  //recorriendo todos los ids obtenidos

  $startRow=2; //inicio de celda
  foreach($listaIdProductos as $value){
...
============================*/
    ...
    $sheet->mergeCells('A'.$startRow.':A'.$startRow+1)
      ...
      ->setCellValue('C'.$startRow+2, 'Saldo anterior al '.$fechaInicial)
      ->setCellValue('D'.$startRow+2, $producto["precio_costo"])//costo x docena
      ->setCellValue('I'.$startRow+2, $saldo)//cantidad en unidades
      ->setCellValue('J'.$startRow+2, $costo_total);//valor
    ...
    $row = $startRow + 3; // Empezar desde la fila 9 para los datos ($startRow = 6 )
    ...
    //inicializando la el siguiente registro
    $startRow=$row + 1;
  }//fin foreach
